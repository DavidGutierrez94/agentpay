...
function buildContext(): string {
  ...
  for (const specialist of ["content", "social", "analytics"] as const) {
    ...
    } catch { /* specialist context may not exist yet */ }
  }
  ...
  } catch { /* dev context may not exist yet */ }
...
async function executeJob(job: ScheduledJob): Promise<void> {
  ...
  try {
    ...
    appendToAgentContext("marketing", { type: "alert", author: "marketing", content: `Job "${job.name}" failed: ${errMsg}` });
  }
}
...
async function main(): Promise<void> {
  ...
  if (mode === "job" && jobId) {
    ...
    if (!job) {
      logger.error(`Job not found: ${jobId}. Available: ${marketingJobs.map((j) => j.id).join(", ")}`);
      process.exit(1);
    }
