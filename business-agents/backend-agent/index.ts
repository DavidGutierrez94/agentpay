...
async function main(): Promise<void> {
  ...
  if (mode === "job" && jobId) {
    ...
    if (!job) {
      logger.error(
        `Job not found: ${jobId}. Available: ${backendJobs.map((j) => j.id).join(", ")}`,
      );
      process.exit(1);
    }
  ...
  if (mode === "interactive") {
    await executeJob({
      ...
      cron: "",
      enabled: true,
      prompt: prompt || "Check for pending backend tasks and work on the highest priority one.",
    });
    return;
  }
