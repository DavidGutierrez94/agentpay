...
async function executeJob(job: ScheduledJob): Promise<void> {
  ...
  try {
    ...
    appendToAgentContext("analytics", {
      type: "alert",
      author: "analytics",
...
async function main(): Promise<void> {
  ...
  if (mode === "job" && jobId) {
    ...
    if (!job) {
      logger.error(
        `Job not found: ${jobId}. Available: ${analyticsJobs.map((j) => j.id).join(", ")}`,
      );
      process.exit(1);
    }
  ...
  if (mode === "interactive") {
    await executeJob({
      ...
      cron: "",
      enabled: true,
      prompt: prompt || "Check for pending analytics tasks and compile the latest metrics.",
    });
    return;
  }
