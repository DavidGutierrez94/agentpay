...
import { contentJobs } from "./schedule.js";
import type { ScheduledJob } from "../shared/types.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const logger = createAgentLogger("content");

const soulPath = path.join(__dirname, "SOUL.md");
const systemPrompt = fs.readFileSync(soulPath, "utf-8");

function buildContext(): string {
  const budget = getBudgetSummary("content");
  const myTasks = listTasks({ assignedTo: "content" });
...
### Dev Updates (for technical content)
...
async function executeJob(job: ScheduledJob): Promise<void> {
  ...
  try {
    ...
    appendToAgentContext("content", { type: "alert", author: "content", content: `Job "${job.name}" failed: ${errMsg}` });
  }
}
...
async function main(): Promise<void> {
  ...
  logger.info(`Starting content agent in ${mode} mode`);

  if (mode === "job" && jobId) {
    const job = contentJobs.find((j) => j.id === jobId);
    if (!job) { logger.error(`Job not found: ${jobId}. Available: ${contentJobs.map((j) => j.id).join(", ")}`); process.exit(1); }
    await executeJob(job);
    return;
  }

  if (mode === "interactive") {
    await executeJob({ id: "interactive", name: "Interactive", cron: "", enabled: true, prompt: prompt || "Check for pending content tasks and work on the highest priority one." });
    return;
  }

  const scheduler = createScheduler("content", executeJob);
  scheduler.register(contentJobs);
  scheduler.start();
  logger.info("Content agent daemon running.");
