...
import { web3Jobs } from "./schedule.js";
import type { ScheduledJob } from "../shared/types.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const logger = createAgentLogger("web3");

const soulPath = path.join(__dirname, "SOUL.md");
const systemPrompt = fs.readFileSync(soulPath, "utf-8");

function buildContext(): string {
  const budget = getBudgetSummary("web3");
  const myTasks = listTasks({ assignedTo: "web3" });
...
async function executeJob(job: ScheduledJob): Promise<void> {
  ...
  try {
      ...
      options: {
        ...
        mcpServers: {
          agentpay: {
            ...
            env: {
              AGENTPAY_KEYPAIR: process.env.WEB3_KEYPAIR || process.env.AGENTPAY_KEYPAIR || "",
    ...
    appendToAgentContext("web3", { type: "alert", author: "web3", content: `Job "${job.name}" failed: ${errMsg}` });
  }
}
...
async function main(): Promise<void> {
  ...
  logger.info(`Starting web3 agent in ${mode} mode`);

  if (mode === "job" && jobId) {
    const job = web3Jobs.find((j) => j.id === jobId);
    if (!job) { logger.error(`Job not found: ${jobId}. Available: ${web3Jobs.map((j) => j.id).join(", ")}`); process.exit(1); }
    await executeJob(job);
    return;
  }

  if (mode === "interactive") {
    await executeJob({ id: "interactive", name: "Interactive", cron: "", enabled: true, prompt: prompt || "Check for pending web3 tasks and work on the highest priority one." });
    return;
  }

  const scheduler = createScheduler("web3", executeJob);
  scheduler.register(web3Jobs);
  scheduler.start();
  logger.info("Web3 agent daemon running.");
